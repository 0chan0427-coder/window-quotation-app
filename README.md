...
## V39
모바일 내부용 상세 견적에서 창호 상세 내역만 가로 스크롤되도록 분리하고, 금액 집계 및 할인 영역은 화면 폭에 맞게 표시되도록 반응형 레이아웃을 개선했습니다.


### V39
- 부가시공비 평형대에 40평대, 50평대 이상 추가
- 가격표 최소 규격 미만은 최소 규격, 최대 규격 초과는 최대 규격으로 자동 계산
- 가격표가 완전한 격자형이 아닌 경우 가까운 상위 규격으로 보정
- 내부용/고객용 견적서에 이미지 저장(PNG) 기능 추가


### V39 가격표 기준 변경
- 업로드된 `제품별 가격표.xlsx`의 각 시트에서 회색으로 표시된 `자재` / `시공` 판매금액 셀을 그대로 추출해 `assets/price-table-v38.json`으로 사용합니다.
- 기존 `자재비` / `시공비` 원가 셀을 합산하지 않습니다.
- 따라서 F-140 2600×2400은 자재 2,024,000원 + 시공 309,000원 = 2,333,000원으로 계산됩니다.
- 이미지 저장 버튼은 견적서가 아직 렌더링되지 않은 경우 현재 상세 견적을 자동으로 렌더링한 뒤 저장을 시도합니다.
