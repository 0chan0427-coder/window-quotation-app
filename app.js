...
const STAFF=[{id:1,name:'한동균 팀장'},{id:2,name:'김민찬 책임'},{id:3,name:'김영찬 책임'}]; const STAFF_KEY='window_quote_staff'; const state={items:[],editingId:null,detailRecord:null,detailItems:[],prices:null,staff:null,estimates:[],hasEstimateNo:false};
...
function recordStaff(r){return r?.staff_name?{id:r.staff_id,name:r.staff_name}:state.staff;}
...
function renderQuote(mode,calc){
  const r=state.detailRecord, customer=mode==='customer';
  ...
  const pay=recordPayment(r)||{deposit:Math.round(total*.2),interim:Math.round(total*.6),balance:Math.round(total*.2),method:'현금'};
  const items=calc.items||[];
  const rows=[];
  for(let i=0;i<18;i++){
    const x=items[i];
    if(x){
      const name=itemName(x)||'-', kind=inferKind(name);
      const outside=['발코니창','복도창'].includes(kind);
      rows.push(`<tr><td>${i+1}</td><td>${outside?'외창':'내창'}</td><td class="left">${name}</td><td class="left">${x.product_code||'-'}</td><td>${x.window_type==='fixed'?'고정창 / 24T':'일반창 / 28T'}</td><td>기본색</td><td>1</td><td>${outside?'AL 방충망':'-'} </td><td>${x.available?Number(x.amount).toLocaleString('ko-KR'):'-'}</td><td>${customer?'':'실측 '+x.actual_width+'×'+x.actual_height}</td></tr>`);
    }else rows.push(`<tr><td>${i+1}</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>`);
  }
  const payRows=`<div class="xlsPayRow total"><div class="payLabel">계 약 총 액</div><div class="payValue">${total.toLocaleString('ko-KR')}</div><div>현금 · BRP · 카드</div></div>
  <div class="xlsPayRow"><div class="payLabel">계약금</div><div class="payValue">${Number(pay.deposit||Math.round(total*.2)).toLocaleString('ko-KR')}</div><div>계약금 (20%)</div></div>
  <div class="xlsPayRow"><div class="payLabel">중도금</div><div class="payValue">${Number(pay.interim||Math.round(total*.6)).toLocaleString('ko-KR')}</div><div>중도금 (60%)</div></div>
  <div class="xlsPayRow"><div class="payLabel">잔 금</div><div class="payValue">${Number(pay.balance||Math.round(total*.2)).toLocaleString('ko-KR')}</div><div>잔 금 (20%)</div></div>`;
  const html=`<div class="quoteTools"><button onclick="window.print()">인쇄 / PDF 저장</button></div>
  <div class="xlsQuote ${customer?'customer':'internal'}">
    <div class="xlsSide"></div>
    <div class="xlsTop">
      <img class="xlsLogo" src="assets/image3.png" alt="LX Z:IN 인테리어 창호 견적서">
      <img class="xlsBadge" src="assets/image4.png" alt="10년 무상보증">
      <div class="xlsNotice">㈜LX하우시스의 완성 창호인 LX Z:IN WINDOW 정품 창호로만 시공합니다.</div>
      <div class="xlsCompany"><img src="assets/image6.png" alt="공식대리점 담당자"><div class="companyStaff">${(recordStaff(r)?.name||state.staff?.name||'담당자')}</div><div class="companyPhone">${recordCustomerPhone(r)||'010-0000-0000'}</div></div>
      <table class="xlsMeta"><tbody>
        <tr><td class="label">견 적 일 자</td><td class="value">${new Date(r.created_at||Date.now()).toLocaleDateString('ko-KR')}</td></tr>
        <tr><td class="label">견 적 번 호</td><td class="value">${estimateNumber(r)}</td></tr>
        <tr><td class="label">현 장 주 소</td><td class="value">${r.address||'-'}</td></tr>
        <tr><td class="label">고 객 정 보</td><td class="value">${r.customer_name||'고객님'} | ${recordCustomerPhone(r)||'010-'}</td></tr>
        <tr><td class="label amount">총 견 적 액</td><td class="value amount">${total.toLocaleString('ko-KR')}</td></tr>
      </tbody></table>
      <div class="xlsValidity">* 본 견적서의 유효 기간은 발행일자로 부터 3개월 이내에 한함</div>
    </div>
    <div class="xlsSection">세 부 견 적 사 항 ( V A T 포 함 가 )</div>
    <table class="xlsItems"><colgroup><col class="no"><col class="kind"><col class="loc"><col class="product"><col class="glass"><col class="color"><col class="qty"><col class="screen"><col class="amount"><col class="memo"></colgroup>
      <thead><tr><th>No.</th><th>구분</th><th>위치</th><th>제품명</th><th>유 리 및 두께</th><th>색상</th><th>수량</th><th>방충망</th><th>금액</th><th>비고</th></tr></thead>
      <tbody>${rows.join('')}</tbody>
      <tbody class="xlsSummary">
        <tr><td colspan="8" class="sumLabel">창 호 계</td><td class="sumAmount">${Number(calc.total||0).toLocaleString('ko-KR')}</td><td></td></tr>
        <tr><td colspan="8" class="sumLabel">표준 시공 기술료</td><td class="sumAmount">0</td><td></td></tr>
        <tr><td colspan="8" class="sumLabel">부가 시공비</td><td class="sumAmount">${Number(extras.total||0).toLocaleString('ko-KR')}</td><td></td></tr>
        <tr><td colspan="8" class="sumLabel">[ 합 계 ]</td><td class="sumAmount">${total.toLocaleString('ko-KR')}</td><td></td></tr>
      </tbody>
    </table>
    <div class="xlsSpacer"></div>
    <div class="xlsBottom">
      <div class="xlsNotes">
        <div class="xlsNoteRow red">LX하우시스 정품 자재로 LX ENG 본사직영시공팀이 시공함 / 10년 품질보증</div>
        <div class="xlsNoteRow">유리 : LX Z:IN 정품 유리로만 설치 시공함</div>
        <div class="xlsNoteRow">잠금장치 : 자동잠금장치 및 크리센트 적용 (규격이하 제품일 경우 제외)</div>
        <div class="xlsNoteRow">방충망 : 외부창 최신 AL분체방충망 적용, 내부창 제외</div>
        <div class="xlsNoteRow">본 상품은 시공성 상품이므로 공사중 시설 훼손 · 스크레치 등이 발생할수 있음.</div>
        <div class="xlsNoteRow red">사양 변경시 금액의 변경 또는 현장 여건상 시공 불가시 계약 취소 할수 있음 (계약금 전액 환불)</div>
        <div class="xlsNoteRow">공식대리점은 시공 전후의 사진을 온라인 상 활용할 수 있음 (부동의시 비게재)</div>
      </div>
      <div class="xlsPayment">
        ${payRows}
        <div class="xlsPayRow"><div class="payLabel">* BRP 진행시</div><div style="grid-column:2 / 4;text-align:left">중도금은 생략될 수 있으며, 잔금은 금융기관을 통해 후불 정산됨</div></div>
        <div class="xlsAccount"><div>입금계좌안내</div><div>452-034930-04-025<br>1005-503-086146</div><div>기업은행<br>우리은행</div><div>㈜도도<br>㈜도도</div></div>
      </div>
    </div>
    <div class="xlsFooter">
      <img class="xlsQr" src="assets/image1.png" alt="공식대리점 QR">
      <img class="xlsDealerLogo" src="assets/image2.png" alt="LX하우시스 공식대리점">
      <div class="xlsFooterText"><b>수도권북부영업센터 공식대리점 주식회사 도도</b> (122-86-46519)<br>대표전화 1577-7864 / www.lx-zin.com / admin@lx-zin.com<br>실내건축면허보유 / LX하우시스 초유의 전부문 최우수상 3관왕 지점</div>
    </div>
    ${customer?`<div class="editablePay"><h3>결제 조건 저장</h3><label>결제 방법<select id="paymentMethod"><option>현금</option><option>계좌이체</option><option>카드</option><option>기타</option></select></label><div class="dims"><label>계약금<input id="payDeposit" type="number"></label><label>중도금<input id="payInterim" type="number"></label><label>잔금<input id="payBalance" type="number"></label></div><button type="button" id="savePayment">결제 조건 저장</button></div>`:''}
  </div>`;
  $('quote').innerHTML=html;$('quote').classList.remove('hidden');
  if(customer){
    ['Deposit','Interim','Balance'].forEach(k=>{const el=$('pay'+k);if(el)el.value=pay[k.toLowerCase()]||0;});
    if($('paymentMethod'))$('paymentMethod').value=pay.method||'현금';
    if($('savePayment'))$('savePayment').onclick=async()=>{
      const payment={deposit:Number($('payDeposit').value)||0,interim:Number($('payInterim').value)||0,balance:Number($('payBalance').value)||0,method:$('paymentMethod').value};
      const payload={payment_data:payment};
      let result=await db.from('estimates').update(payload).eq('id',r.id);
      if(result.error){
        const memo=cleanMemo(r.memo);
        result=await db.from('estimates').update({memo:encodeExtras(memo,recordStaff(r),({phone:recordCustomerPhone(r)}),payment)}).eq('id',r.id);
      }
      if(result.error)alert(result.error.message);else{r.payment_data=payment;alert('결제 조건이 저장되었습니다.');}
    };
  }
}
