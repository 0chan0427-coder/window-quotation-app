...
function discountInfo(calc,extras){const totals=materialInstallTotals(calc);const rate=Math.max(0,Math.min(100,Number(extras?.discountRate)||0));const rateMaterial=Number(calc?.rateMaterialTotal)||totals.material;const discount=Math.round(rateMaterial*rate/100);const windowTotal=Number(calc?.total)||0;const hasExecution=Array.isArray(calc?.items)&&calc.items.some(x=>x.pricing_source==='execution');const constructionExtra=hasExecution?0:Number(extras?.constructionTotal||extras?.total||0);const extraProducts=Number(extras?.additionalProductTotal||0);const extraTotal=constructionExtra+extraProducts;return {...totals,rate,rateMaterial,discount,windowTotal,constructionExtra,extraProducts,extraTotal,finalTotal:windowTotal-discount+extraTotal};}
...
async function calculate(items,rates=[],conditions={}){
  ...
  for(let index=0;index<items.length;index++){
    ...
    const rate=Number(item.pricing_rate??rates[index]??0),rateApplied=Number.isFinite(rate)&&rate>0; let execution=null;
    try{execution=await findExecutionPrice({...item,product_code:lookup},conditions);}catch(err){console.warn(err);}
    if(execution){
      ...
      const effectiveRate=rateApplied?rate:(Number(execution.default_rate)>0?Number(execution.default_rate):1);
      ...
      out.push({...item,lookup,available:amount>0,applied_width:w,applied_height:h,material:k+l,install,raw_amount:rawAmount,amount,pricing_rate:effectiveRate,rate_applied:true,pricing_source:'execution',execution_sheet:execution.execution_sheet,execution_components:{complete_window_dc:k,glass_dc:l,installation:m,equipment_demolition_tile:n,survey_molding_protection:o,rate_material:rateMaterial}});continue;
    }
    ...
    total+=amount;rateMaterialTotal+=rateMaterial;out.push({...item,lookup,available:true,applied_width:appliedWidth,applied_height:appliedHeight,material:baseMaterial,install:baseInstall,amount,pricing_rate:rateApplied?rate:null,base_amount:baseMaterial+baseInstall,rate_applied:rateApplied,pricing_source:'price-table'});
  }
...
$('estimateForm').onsubmit=async e=>{e.preventDefault();if($('saveBtn').disabled)return;$('saveBtn').disabled=true;$('message').textContent='가격 확인 중...';try{
  const extras=getExtras(),ap=readAdditionalProducts();extras.additionalProducts=ap;extras.additionalProductTotal=ap.reduce((s,x)=>s+x.finalAmount,0);extras.total=extras.total+extras.additionalProductTotal;
  ...
  const rawItems=state.items.map((el,index)=>{const kind=el.querySelector('.kind').value;const name=kind==='직접입력'?el.querySelector('.customName').value:el.querySelector('.location').value;const product=el.querySelector('.product').value;const actualWidth=Number(el.querySelector('.w').value),actualHeight=Number(el.querySelector('.h').value),pricingRate=Number(el.querySelector('.pricingRate').value);return{installation_location:name,window_kind:kind,screen_option:el.querySelector('.screen').value,product_code:product,window_type:el.querySelector('.windowType').value==='고정창'?'fixed':'normal',actual_width:actualWidth,actual_height:actualHeight,applied_width:Math.ceil(actualWidth/200)*200,applied_height:Math.ceil(actualHeight/200)*200,pricing_rate:Number.isFinite(pricingRate)&&pricingRate>0?pricingRate:null,sort_order:index+1};});
  if(!rawItems.length)throw new Error('창호 항목을 하나 이상 추가해주세요.');extras.windowRates=rawItems.map(x=>x.pricing_rate||null);
  $('message').textContent='실행가 계산 중...';const calc=await calculate(rawItems,extras.windowRates,extras.conditions||{});const unavailable=calc.items.find(x=>!x.available);if(unavailable)throw new Error(unavailable.reason||'해당 치수의 가격표가 없습니다.');
  ...
  $('message').textContent=ratesChanged?'저장되었습니다. 변경된 요율로 금액을 다시 계산했습니다.':'저장되었습니다.';setTimeout(()=>{show('home');load();},500);
}catch(err){console.error(err);$('message').textContent='저장 실패: '+(err?.message||err);}finally{$('saveBtn').disabled=false;}};
