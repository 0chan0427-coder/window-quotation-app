...
function resetForm(){$('estimateForm').reset();$('items').innerHTML='';state.items=[];state.editingId=null;$('formTitle').textContent='새 견적 작성';$('formEyebrow').textContent='NEW ESTIMATE';$('saveBtn').textContent='견적 저장';$('message').textContent='';if($('estimateColor'))$('estimateColor').value='기본색';}
...
function recordExtras(r){let ex=r?.extras_data||{};if(typeof ex==='string'){try{ex=JSON.parse(ex)}catch{ex={}}}return {...{equipment:0,demolition:0,sash:0,molding:0,protection:0,total:0,color:'기본색',discountRate:0},...ex};}
...
function customerFinalAmount(r,internalTotal){const ex=recordExtras(r)||{};const manual=Number(ex.customerFinalAmount);if(Number.isFinite(manual)&&manual>0)return Math.round(manual/10000)*10000;return Math.round(Number(internalTotal||0)/10000)*10000;}
function normalizedManualAmount(value){const n=Number(value);if(!Number.isFinite(n)||n<=0)return null;return Math.round(n/10000)*10000;}
...
async function saveQuoteAsImage(mode){
  let target=$(mode==='internal'?'.internalQuoteSheet':'.xlsQuote.customer');
  if(!target && state.detailRecord && state.detailItems?.length){
    try{
      const calc=await calculate(state.detailItems);
      renderQuote(mode,calc);
      target=$(mode==='internal'?'.internalQuoteSheet':'.xlsQuote.customer');
    }catch(err){
      console.error(err);
...
function renderInternalQuote(calc){
  ...
  $('quote').innerHTML=`<div class="quoteTools"><button onclick="window.print()">인쇄 / PDF 저장</button><button class="imageSaveBtn secondary" type="button" onclick="saveQuoteAsImage('internal')">이미지 저장</button></div>
    <div class="internalQuoteSheet">
      ...
      <h3>창호 상세 내역</h3><div class="internalTable detailEstimateTable"><table><thead><tr><th>No.</th><th>구분</th><th>위치</th><th>제품</th><th>실측 가로 × 세로(mm)</th><th>적용 가로 × 세로(mm)</th><th>형태</th><th>자재비</th><th>시공비</th><th>합계</th></tr></thead><tbody>${rows||'<tr><td colspan="10">등록된 창호 항목이 없습니다.</td></tr>'}</tbody></table></div>
      <h3>금액 집계 및 할인</h3><div class="internalTable internalSummaryTable"><table><tbody><tr><th>자재비 합계</th><td class="num">${money(di.material)}</td></tr><tr><th>시공비 합계</th><td class="num">${money(di.install)}</td></tr><tr><th>창호 금액 합계</th><td class="num">${money(di.material+di.install)}</td></tr><tr><th>자재비 할인율</th><td class="num"><input id="discountRate" type="number" min="0" max="100" step="0.1" value="${di.rate}" style="width:90px;text-align:right"> %</td></tr><tr><th>자재비 할인금액</th><td class="num" id="discountAmount">${money(di.discount)}</td></tr><tr><th>부가시공비 합계</th><td class="num">${money(extras.total||0)}</td></tr><tr class="internalSum"><th>할인 적용 후 최종 견적금액</th><td class="num" id="internalFinalTotal">${money(total)}</td></tr></tbody></table></div><div style="margin:10px 0 18px;text-align:right"><button type="button" id="saveDiscount">할인율 저장</button></div>
