...
$('estimateForm').onsubmit=async e=>{e.preventDefault();if($('saveBtn').disabled)return;$('saveBtn').disabled=true;$('message').textContent='가격 확인 중...';try{const extras=getExtras();const rawMemo=$('memo').value;let originalAuthor=null;if(state.editingId&&state.detailRecord)originalAuthor=recordStaff(state.detailRecord);const rawItems=state.items.map(el=>{const kind=el.querySelector('.kind').value;const name=kind==='직접입력'?el.querySelector('.customName').value:el.querySelector('.location').value;const product=el.querySelector('.product').value;const actualWidth=Number(el.querySelector('.w').value);const actualHeight=Number(el.querySelector('.h').value);return{installation_location:name,window_kind:kind,screen_option:el.querySelector('.screen').value,product_code:product,window_type:el.querySelector('.windowType').value==='고정창'?'fixed':'normal',actual_width:actualWidth,actual_height:actualHeight,applied_width:Math.ceil(actualWidth/200)*200,applied_height:Math.ceil(actualHeight/200)*200};});if(!rawItems.length)throw new Error('창호 항목을 하나 이상 추가해주세요.');$('message').textContent='가격표 조회 중...';const calc=await calculate(rawItems);const unavailable=calc.items.find(x=>!x.available);if(unavailable)throw new Error(unavailable.reason||'해당 치수의 가격표가 없습니다.');const payload={project_name:$('projectName').value,customer_name:$('customerName').value,address:$('address').value,status:$('status').value,memo:cleanMemo(rawMemo),customer_phone:$('customerPhone').value,staff_id:(originalAuthor||state.staff)?.id||null,staff_name:(originalAuthor||state.staff)?.name||'',extras_data:extras,total_amount:calc.total+extras.total};
